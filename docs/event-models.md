...
| Tidal disruption | The live stress indicator uses `T = 2 M_BH R³ / (m d³)`. Pre-contact deformation preserves volume; capture seeds bound and escaping tracer branches from the measured encounter frame. | [Phantom](https://phantomsph.github.io/) is GPLv3 astrophysical SPH/MHD software with black-hole accretion and tidal-disruption setups. [NASA's published supercomputer TDE sequence](https://svs.gsfc.nasa.gov/14000) is the qualitative morphology reference. | Keep the current stream explicitly massless. Use a parameterized Phantom result set only after stellar profile, penetration factor, mass ratio, relativistic potential, fallback, and self-intersection assumptions are recorded. |
