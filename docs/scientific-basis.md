...
The compact-object renderer has three explicit states: a naked black hole with no local emitter, an accreting black hole with a thin disk, and an active nucleus with the same disk plus qualitative bipolar jets. The accreting and active-nucleus presets place these objects in the same N-body state as their nearby stars and planets. Gravity comes only from the listed masses. Disk emission and jets neither add mass nor exert a force. Capture retains the emitter state of the black hole instead of silently converting an accreting object into a naked one. The detailed Quasar Engine laboratory separately evaluates the zero-torque thin-disk temperature model.

Neutron stars and pulsars are ordinary massive N-body participants with deliberately enlarged render radii and compact physical radii. The pulsar's 1.337-second spin is converted into the model time unit. Closed vacuum-dipole guides obey `r = L sin²θ` only inside the normalized light cylinder. In a rotating plasma magnetosphere the field separates into a finite closed zone and open polar flux; the open lines become an approximately radial, swept-back wind and an equatorial current sheet. Those open layers are rendered to the scene boundary, which is the finite numerical representation of an outflow that continues far beyond the viewport. Extending the closed dipole loops to infinity would contradict the force-free CKF topology. Polar caps and feathered cones retain the magnetic-axis lighthouse interpretation of pulsations. These layers are a field-topology visualization, not a solution of the pulsar equation, pair cascades, radiation transport, or plasma feedback. The double-neutron-star preset is Newtonian and omits gravitational-wave inspiral and tidal deformation. Contact can trigger a reduced-order hot outflow, but the remnant type, equation of state, kilonova emission, and relativistic merger dynamics are not solved.
...
For two equal-density, equal-radius worlds this gives twice the mass and `∛2 ≈ 1.26` times the radius. The rendered result is a slightly oblate, volume-equivalent post-impact core with a fractured thermal surface. It does not claim a calculated shape, temperature, or cooling curve.

Each collision event selects one of five bounded GPU visual regimes: planetary impact, stellar merger, tidal disruption, horizon capture, or compact merger. Their origins and axes come from the measured contact position, normal, relative velocity, participant radii, and colours. Planetary impacts produce a shock/ejecta curtain. Stellar mergers produce an equatorial spiral outflow around an expanded photosphere. Tidal disruption and horizon capture produce asymmetric bound and escaping streams. A vacuum black-hole merger emits no invented luminous matter.

The stored planetary ejecta scale is `0.5 v_impact`, following the central tendency reported for escaping debris in the cited giant-impact database; its ratio to mutual escape speed sets a capped visual expansion. All tracers are massless and do not feed back into gravity. Their lifetimes, colours, counts, cooling, and individual paths are reduced-order visual encodings. Cratering, material strength, phase changes, atmosphere loss, chemistry, fragment-size distribution, reaccretion, stellar mixing, radiative transfer, and compact-object equations of state are not calculated. Credible resolved debris requires a fluid method such as SPH; late compact mergers require numerical relativity. The implementation and open upgrade candidates are catalogued in [Event model sources](event-models.md).
...
Escaping rays sample NASA's plate carrée Deep Star Maps 2020 celestial map. The celestial orientation places its resolved Milky Way region inside the default camera view; a nonlinear HDR exposure strengthens catalogue stars while retaining a black floor. Captured rays terminate in the shadow. A razor-thin disk from `3 r_s` to `12 r_s` can intersect a ray twice, which produces camera-dependent direct and secondary images of its front and rear faces above and below the shadow. Disk brightness includes a zero-torque radial temperature profile, approximate colour mapping, orbital time delay, and relativistic frequency shift. The shader uses the bolometric `g⁴` intensity transform: Liouville's monochromatic `I_ν/ν³` invariant contributes `g³`, and integration over shifted frequency contributes one more factor of `g`. The display radiance is deliberately normalized for visibility because the sandbox does not specify an absolute accretion rate or observing band; the `g⁴` approach/recession ratio is retained while the shadow itself remains black. Procedural density filaments keep the idealized disk from appearing as a static image.

The previous whole-screen thin-lens pass was removed because it distorted the already beam-traced image a second time and made a large unphysical ripple. The bounded transfer surface currently lenses the catalogue sky and its own disk, not trails or nearby 3D bodies. The model is Schwarzschild: it assumes a static non-rotating hole and static observer. It does not implement Kerr frame dragging, a moving-observer tetrad, full spectral radiative transfer, polarization, disk thickness, self-shadowing, coronae, magnetohydrodynamics, or GRMHD jets. The event horizon is not drawn as a surface; the visible dark region is the larger optical shadow. The separate Black Hole Optics laboratory remains the equation-level ray reference.

## Visual model and detail levels

Earth, Venus, Mars, and Jupiter use credited NASA/JPL observational maps. The background uses 2K, 4K, and 8K variants of NASA's linear-HDR Deep Star Maps 2020 celestial projection, derived from Gaia DR2, Tycho-2, and Hipparcos-2 catalogues. Stars, generic rocky bodies, Saturn, ice giants, atmospheres, disks, jets, remnants, and collision tracers use procedural visual materials. These materials communicate object class but are not time-resolved observations. The sky is non-interacting visual context and is not a member of the active N-body state.
...
- NASA/Goddard Space Flight Center Scientific Visualization Studio, [Black Hole Accretion Disk Visualization](https://svs.gsfc.nasa.gov/13326/); viewing-angle dependence, direct and lensed disk images, photon-ring anatomy, and relativistic Doppler-beaming reference.
- NASA/Goddard Space Flight Center Scientific Visualization Studio, [Deep Star Maps 2020](https://svs.gsfc.nasa.gov/4851); plate carrée HDR map of 1.7 billion catalogue stars, with Gaia DR2 credited to ESA/Gaia/DPAC.
...
- I. Contopoulos, D. Kazanas, and C. Fendt, [The Axisymmetric Pulsar Magnetosphere](https://arxiv.org/abs/astro-ph/9903049), *The Astrophysical Journal* 511 (1999), 351–358; force-free separation between the closed zone and open wind across the light cylinder.
- NASA/Goddard Space Flight Center Scientific Visualization Studio, [Pulsar Current Sheets — Magnetic Field Solution](https://svs.gsfc.nasa.gov/4638/); force-free/PIC magnetosphere morphology used to distinguish closed field, swept-back open field, and current-sheet layers.
